...
module.exports = app => {
  ...
  async function getChannelNameTest(network, keyValueStorePath, channelName) {
    const helper = await fabricHelper(network, keyValueStorePath, channelName);
    const client = await getClientForOrg('org1', helper.clients);
    let result = client.getChannel(channelName);
    console.log(result);
  }
  ...
  async function getPeersForChannel(network, keyValueStorePath, channelName, orgName){
      const helper = await fabricHelper(network, keyValueStorePath, channelName);
      const client = await helper.clients[orgName];
  ...
  app.getChannelNameTestV1_0 =getChannelNameTest;
  ...
  app.getPeersForChannelV1_0 = getPeersForChannel;
