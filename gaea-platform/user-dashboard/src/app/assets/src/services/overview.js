...
export async function queryTransactionRealtime(params) {
  return request(`/v2/txrealtime/${params.channel_id}/${params.minutes}`);
}

export async function queryBlockByNumber(params) {
  return request(`/v2/blocks/${params.channel}?number=${params.blockNum}`);
}

export async function queryBlockByTime(params) {
    return request(`/v2/blocks/${params.channel}?times_begin=${params.startTime}&times_end=${params.endTime}`);
}